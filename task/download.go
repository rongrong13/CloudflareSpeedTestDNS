...
// DownloadProgressCallback 下载测速进度回调函数类型
// 参数：队列总数, 已完成数, 当前测速结果（已排序）
type DownloadProgressCallback func(queueTotal, current int, results utils.DownloadSpeedSet)

// DownloadProgressCallbackFunc 全局下载进度回调函数，由 main 包设置
var DownloadProgressCallbackFunc DownloadProgressCallback
...
func TestDownloadSpeed(ipSet utils.PingDelaySet) (speedSet utils.DownloadSpeedSet) {
	...
	// 使用局部变量，避免修改全局 TestCount 导致多轮测试时行为异常
	testCount := TestCount
	...
	for i := 0; i < testNum; i++ {
		...
		// 每个 IP 测试完都通知进度（在 break 之前，确保最后一条也发送）
		if DownloadProgressCallbackFunc != nil {
			DownloadProgressCallbackFunc(testNum, i+1, speedSet)
		}
